...
public class Invoice {
	...
	public LinkedHashMap creditbuyerList()
	{
		Inventory inv =new Inventory(); 
		return(inv.custList("B"));
	}
	...
	public String billNumber()
	{
		...
		{//
			if((a=mysql.selectValue("Max(Bill_No)",null,"sales_table"))!=null)
			{
				mysql.closeDB("LCI");
	...
	public String sell(Hashtable<String,String> Hashdata) 
	{
		...
		{//
			...
			if((bill_no=mysql.addNewRow(ALcol,ALdata,"sales_table",true))!=null)
			{
				...
				for(int i=0;i<5 && (!count[i].equals("0"));i++)
				{
					...
					/*sql.exeUpdate("insert into lci.sales_details_table (bill_no,count_id,quantity,rate) values("+bill+","
				             +s+"," +quantity[i].getText()+","+rate[i].getText()+")");*/
					
					mysql.addNewRow(ALcol,ALdata,"sales_details_table",false);
